...
SHA384& SHA384::Write(const unsigned char *data, size_t len) {
    const unsigned char* end = data + len;
    size_t bufsize = bytes % 128;
    if (bufsize && bufsize + len >= 128) {
        memcpy(buffer + bufsize, data, 128 - bufsize);
        bytes += 128 - bufsize;
        data += 128 - bufsize;
...
void SHA384::Finalize(unsigned char hash[outputLen])
{
    ...
    Write(sizedesc, 16);
    WriteBE64(hash, state[0]);
    WriteBE64(hash + 8, state[1]);
    WriteBE64(hash + 16, state[2]);
    WriteBE64(hash + 24, state[3]);
    WriteBE64(hash + 32, state[4]);
    WriteBE64(hash + 40, state[5]);
    WriteBE64(hash + 48, state[6]);
    WriteBE64(hash + 56, state[7]);
}
